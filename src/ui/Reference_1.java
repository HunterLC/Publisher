...
public class Reference_1 extends JFrame {
	...
	public Reference_1() {
		...
		titledBorder = new TitledBorder(UIManager.getBorder("TitledBorder.border"), "共"+ReferenceDao.getInstance().QueryCount()+"项", TitledBorder.LEADING, TitledBorder.TOP, null, Color.RED);
		...
		listTable.addMouseListener(new MouseAdapter() {   //用户名表格点击事件
			...
			public void mouseClicked(MouseEvent arg0) {
				...
				CURRENTID = ReferenceDao.getInstance().QueryIDByName(shum);//获得当前选中编辑室的id
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(shum.equals("")) {//编辑室名为空
					...
					if(CURRENTID !=0 ){  //当前选中了一项
						if(!ReferenceDao.getInstance().QueryShumByID(CURRENTID).equals(shum) && ReferenceDao.getInstance().shumIsExist(shum))
							JOptionPane.showMessageDialog(null,"该图书名已经存在","提示",JOptionPane.INFORMATION_MESSAGE);
						else//修改之后的排序号、编辑室不存在					
							ReferenceDao.getInstance().Update(IdentifierDao.getInstance().QueryIDByName(bjsName),CodeDao.getInstance().QueryIDByName(bookType),shum,CURRENTID);
					}
					else if(CURRENTID ==0 ){  //添加编辑室
						if(ReferenceDao.getInstance().shumIsExist(shum))
							JOptionPane.showMessageDialog(null,"该图书名已经存在","提示",JOptionPane.INFORMATION_MESSAGE);
						else {
							ReferenceDao.getInstance().Add(IdentifierDao.getInstance().QueryIDByName(bjsName),CodeDao.getInstance().QueryIDByName(bookType),shum);
							CURRENTID =ReferenceDao.getInstance().QueryIDByName(shum);//更新当前id指针为新建用户的id
							deleteButton.setEnabled(true);
							resetButton.setEnabled(true);
							titledBorder1.setTitle("修改");
							settingPanel.repaint();
		...
		deleteButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int selection = JOptionPane.showConfirmDialog(null,"确认删除"+ReferenceDao.getInstance().QueryShumByID(CURRENTID)+"？","删除",JOptionPane.OK_CANCEL_OPTION,JOptionPane.QUESTION_MESSAGE);
				if( selection == JOptionPane.OK_OPTION ){
					String shum = nameTextField.getText();
					ReferenceDao.getInstance().Delete(shum);
    ...
    public Object[][] queryData(){
        List<Reference> list=ReferenceDao.getInstance().QueryAllByMethod1();
